...
public class MessageController {
    ...
    @GetMapping("/messages")
    Message getMessage(@RequestParam User login, Message message) {
        try {
            return messageService.getUserMessagesByLogin(login, message);
    ...
    public ResponseEntity<Message> createMessages(@RequestBody MessageDto messageDto) {
        Message messageCreated = messageService.createMessage(messageDto);
        if (messageCreated == null) {
            new MessageNotCreateException();
        }
        return ResponseEntity.ok(messageCreated);
    }
}

////messageService.getUserMessagesByLogin(login, message);